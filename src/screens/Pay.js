import React, { useState, useEffect } from 'react'
import { View, Text, TextInput, ScrollView, Image, Button, TouchableOpacity } from 'react-native'
import { useDispatch, useSelector } from "react-redux";

import Ionicons from 'react-native-vector-icons/Ionicons'

export default function Pay({ route, navigation }) {
    const { img, name, price, quantity } = route.params;
    const [color, setColor] = useState("black");
    const [color1, setColor1] = useState("black");
    const [color2, setColor2] = useState("black");
    const [color3, setColor3] = useState("black");

    const onChange = () => {
        setColor("green");
        setColor2("black")
        setColor3("black")
        setColor1("black")
    };
    const onChange1 = () => {
        setColor1("green");
        setColor("black")
        setColor2("black")
        setColor3("black")
    };
    const onChange2 = () => {
        setColor2("green");
        setColor1("black")
        setColor3("black")
        setColor("black")
    };
    const onChange3 = () => {
        setColor3("green");
        setColor("black")
        setColor2("black")
        setColor1("black")
    };
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            <View style={{ flex: 1, backgroundColor: '#EFEFEF' }}>
                <ScrollView showsVerticalScrollIndicator={false}>
                    <View style={{}}>
                        <View style={{ backgroundColor: 'white', padding: 10 }}>
                            <Text style={{ fontSize: 18, fontWeight: 'bold' }} >Thông tin giao hàng</Text>
                            <Text style={{ marginTop: 15, fontSize: 13, color: "grey" }}>Tên người nhận</Text>
                            <TextInput placeholder="Họ và tên" style={{ borderBottomWidth: 1, fontWeight: 'bold', fontSize: 17 }} />
                            <Text style={{ marginTop: 15, fontSize: 13, color: "grey" }}>Số điện thoại</Text>
                            <TextInput placeholder="Nhập số điện thoại" style={{ borderBottomWidth: 1, fontWeight: 'bold', fontSize: 17 }} keyboardType="numeric" />
                            <Text style={{ marginTop: 15, fontSize: 13, color: "grey" }}>Địa chỉ giao hàng</Text>
                            <TextInput placeholder="Giao hàng tới địa chỉ" style={{ borderBottomWidth: 1, fontWeight: 'bold', fontSize: 17 }} />
                            <Text style={{ fontSize: 18, marginTop: 30, fontWeight: 'bold' }} >Ghi chú</Text>
                            <TextInput placeholder="Nội dung ghi chú" style={{ borderColor: 'grey', borderWidth: 1, fontSize: 17, marginTop: 10, borderRadius: 10, padding: 10 }} />
                        </View>
                        <View style={{ marginTop: 10, backgroundColor: 'white', padding: 10 }}>
                            <Text style={{ fontSize: 18, marginTop: 30, fontWeight: 'bold' }} >Chi tiết đơn hàng</Text>
                            <View style={{ flexDirection: 'row', flex: 1, marginTop: 15 }}>
                                <View style={{ flex: 2.5 }}>
                                    <Image source={{ uri: img }} style={{ width: 80, height: 80, borderRadius: 10 }} />
                                </View>
                                <View style={{ flex: 4.5 }}>
                                    <Text style={{ marginTop: 10, fontSize: 15, fontWeight: 'bold' }}>{name}</Text>
                                    <Text style={{ marginTop: 10, fontSize: 15, fontWeight: 'bold' }}>SL: {quantity}</Text>
                                </View>
                                <View style={{ flex: 3 }}>
                                    <Text style={{ fontSize: 18, color: 'green', textAlign: "right", marginTop: 10 }}>{price * quantity} đ</Text>
                                </View>
                            </View>
                        </View>
                        <View style={{ marginTop: 10, backgroundColor: 'white', padding: 10 }}>
                            <Text style={{ fontSize: 18, marginTop: 30, fontWeight: 'bold' }} >Hình thức thanh toán</Text>
                            <TouchableOpacity onPress={onChange}  >
                                <View style={{ flexDirection: 'row', flex: 1, marginTop: 15 }}>
                                    <View style={{ flex: 2, justifyContent: 'center' }}>
                                        <Image source={{ uri: "data:image/png;base64," }} style={{ width: 50, height: 20, borderRadius: 5 }} />
                                    </View>
                                    <View style={{ flex: 8, justifyContent: 'center', }}>
                                        <Text style={{ marginTop: 10, fontSize: 15, fontWeight: 'bold', color: color }}>Thanh toán trả sau</Text>
                                    </View>
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={onChange1}>
                                <View style={{ flexDirection: 'row', flex: 1, marginTop: 15 }}>
                                    <View style={{ flex: 2, justifyContent: 'center' }}>
                                        <Image source={{ uri: "https://upload.wikimedia.org/wikipedia/vi/archive/f/fe/20201011055543%21MoMo_Logo.png" }} style={{ width: 50, height: 50 }} />
                                    </View>
                                    <View style={{ flex: 8, justifyContent: 'center' }}>
                                        <Text style={{ marginTop: 10, fontSize: 15, fontWeight: 'bold', color: color1 }}>Thanh toán ví MOMO</Text>
                                    </View>
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={onChange2}>
                                <View style={{ flexDirection: 'row', flex: 1, marginTop: 15 }}>
                                    <View style={{ flex: 2, justifyContent: 'center' }}>
                                        <Image source={{ uri: 'https://storage.googleapis.com/partnership-shopee-files-live/live/airpay/filer_public_thumbnails/filer_public/b1/8c/b18cd405-3927-460f-a03c-5229199add79/logo-apa_400x400.png__300x300_q95_crop_subsampling-2.png' }} style={{ width: 50, height: 50, borderRadius: 10 }} />
                                    </View>
                                    <View style={{ flex: 8, justifyContent: 'center' }}>
                                        <Text style={{ marginTop: 10, fontSize: 15, fontWeight: 'bold', color: color2 }}>Thanh toán ví Airpay</Text>
                                    </View>
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={onChange3}>
                                <View style={{ flexDirection: 'row', flex: 1, marginTop: 15 }}>
                                    <View style={{ flex: 2, justifyContent: 'center' }}>
                                        <Image source={{ uri: 'https://play-lh.googleusercontent.com/u5pV499kLMi9rZs8vxI9IOIQKKFcj2Ae_-vlJDuMc31GPeRx40BOgpIVqXvgiLdCgg' }} style={{ width: 50, height: 50, borderRadius: 10 }} />
                                    </View>
                                    <View style={{ flex: 8, justifyContent: 'center' }}>
                                        <Text style={{ marginTop: 10, fontSize: 15, fontWeight: 'bold', color: color3 }}>Thanh toán ví VNPAY</Text>
                                    </View>
                                </View>
                            </TouchableOpacity>
                        </View>
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 10, backgroundColor: 'white', padding: 10 }}>
                        <View><Text style={{ fontSize: 18, fontWeight: 'bold' }} >Tổng cộng</Text></View>
                        <View><Text style={{ fontSize: 22, fontWeight: 'bold', color: 'green' }} >{quantity * price} đ </Text></View>
                    </View>
                </ScrollView>
            </View>
            <View style={{ marginBottom: 40 }}>
                {/* <Button

                    title="Thanh toán"
                    color="#841584"
                    
                /> */}
                <View style={{ justifyContent: 'center', alignItems: 'center', backgroundColor: 'white' }}>
                    <TouchableOpacity

                        style={{
                            marginTop: 10,
                            backgroundColor: '#FECE02',
                            width: '96%', borderWidth: 1,
                            borderRadius: 10, paddingVertical: 12,
                            height: 50,
                            borderColor: 'transparent',
                            justifyContent: 'center',
                            alignItems: 'center'
                        }}>
                        <Text style={{ color: '#000000', fontWeight: 'bold' }}>Đặt Hàng!</Text>
                    </TouchableOpacity>
                </View>
            </View>
        </View>
    )
}
